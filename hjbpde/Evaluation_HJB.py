...
def evaluate_hjb_solution():
    ...
    try:
        ...
        print(f"Error: Model file not found at {e.filename}. Please ensure the script is in the same directory as the .pth files.")
        return
    ...
    rmse_J = np.sqrt(np.mean((pred_J_flat - true_J_flat)**2))
    range_J = np.max(true_J_flat) - np.min(true_J_flat)
    nrmse_J = rmse_J / range_J
    ...
    print("=" * 50)
    print(f"Relative L2 Error for Value Function J:   {l2_error_J:.6e}")
    print(f"Normalized RMSE for Value Function J:     {nrmse_J:.6e}")
    print("\n--- Optimal Control u Comparison ---")
    print(f"  Predicted Solution (Mean):   {model_u_mean:.6f}")
    print(f"  Predicted Solution (Std Dev): {model_u_std:.6e}  (A smaller value indicates the solution is closer to a constant)")
    print(f"  Analytical Solution:         {true_u_val:.6f}")
    print("=" * 50)
